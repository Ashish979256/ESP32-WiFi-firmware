...
char server_cloud[] = "www.holmiumtechnologies.com";          // name address for Google (using DNS)
...
void initWiFi();                                           // initiates wifi connection
...
// reconnects the disconnected wifi connection
...
bool uploadRestartData();                            //  upload restart data to server according to gsm/wifi/ethernet flag
...
void notFound(AsyncWebServerRequest *request);      // This response is given to local sever 
void initWiFiAP();                                  // it enable wifi in Access point mode
bool startServer();                                 // it start a local sever
void setup() {
  Serial.begin(9600);
  ...
  if(!checkForInternet())
  {
    Serial.println("trying to get internet");
  ...
  if ((WiFi.status()==WL_CONNECTED || flag.is_gsm==1 || flag.is_wifi==1))
  {
    uint8_t count=0;
    while (!uploadRestartData())
    {
      count=count+1;
      if(count==5)
       {
        ...
        Serial.println("sd flag is disable");
        break;
       }
      Serial.println("failed to upload trying again");
      delay(5000);
    }
  }
  ...
  is_correct_config=true;   //remove this comment after testing
}
void loop(){
  if (flag.is_active==1)
  {
    if (is_correct_config==true)
    {
      ...
      Serial.println("Incorrect configuration of logger");
  }
  else
    Serial.println("flagActive is disable "); 
}
...
{  
   ...
   int count_fail=0;
   do {
    conn = client.connect(server_cloud,80);
    if (conn!=1)
    {
      ++count_fail;
      client.stop();
...
bool saveNetworkConfig()
{
  ...
  strcat(network_str,network.gateway_wifi); 
  strcat(network_str,"\n");
...
bool uploadRestartData()
{
  Serial.println("\nuploadRestartData Function ");
  char Data[50]="";
  char response[250]="";
  strcpy(Data,"LD=");
  append(Data,logger.logger_id,",");
...
int readAndSave()
{
  ...
  for (uint8_t i=0;i<logger.device_count;i++)
  {
    ...
    {
      Serial.println("SD card flag is enabled");
      }
    else
    {
      if (WiFi.status() == WL_CONNECTED)
...
void initWiFiAP()
{
   Serial.println("\n Entered in initWiFiAP function");
   bool correct_data =false;
   WiFi.mode(WIFI_AP_STA);
  //WiFi.begin(ssid, password);
  WiFi.softAP(default_ssid_server, default_password_server);
...
bool startServer()
{
  Serial.println("\nEntered in startServer function");
  ...
  request->send(SPIFFS, "/logo.jpg", "text/jpg");
  });
...
*/
  ...
  server.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request) {
    ...
    if (request->hasParam(PARAM_INPUT_1)&&request->hasParam(PARAM_INPUT_2)&&request->hasParam(PARAM_INPUT_3)&&request->hasParam(PARAM_INPUT_4)&&request->hasParam(PARAM_INPUT_5)&&request->hasParam(PARAM_INPUT_6)&&request->hasParam(PARAM_INPUT_7) &&request->hasParam(PARAM_INPUT_8)&&request->hasParam(PARAM_INPUT_9)) {
      input_message = request->getParam(PARAM_INPUT_1)->value()+','+request->getParam(PARAM_INPUT_2)->value()+','+request->getParam(PARAM_INPUT_3)->value()+','+request->getParam(PARAM_INPUT_4)->value()+','+request->getParam(PARAM_INPUT_5)->value()+','+request->getParam(PARAM_INPUT_6)->value()+','+request->getParam(PARAM_INPUT_7)->value()+','+request->getParam(PARAM_INPUT_8)->value()+','+request->getParam(PARAM_INPUT_9)->value();
      strcpy(testMode,String(request->getParam(PARAM_INPUT_1)->value()).c_str());
      strcpy(dhcpEnable,String(request->getParam(PARAM_INPUT_2)->value()).c_str());
      strcpy(ssid,String(request->getParam(PARAM_INPUT_3)->value()).c_str());
      strcpy(password,String(request->getParam(PARAM_INPUT_4)->value()).c_str());
      strcpy(ip,String(request->getParam(PARAM_INPUT_5)->value()).c_str());
      strcpy(gateway,String(request->getParam(PARAM_INPUT_6)->value()).c_str());
      strcpy(subnet,String(request->getParam(PARAM_INPUT_7)->value()).c_str());
      strcpy(priDNS,String(request->getParam(PARAM_INPUT_8)->value()).c_str());
      strcpy(secDNS,String(request->getParam(PARAM_INPUT_9)->value()).c_str());
      Serial.print("test mode = ");
      Serial.println(testMode);
      Serial.print("dhcp enable = ");
      Serial.println(dhcpEnable);
      Serial.print("ssid = ");
      Serial.println(ssid);
      Serial.print("password =");
      ...
      Serial.print("priDNS = ");
      Serial.println(priDNS);
      Serial.print("secDNS = ");
      Serial.println(secDNS);
    ...
    request->send(200, "text/html", "HTTP GET request sent to your ESP on input field with value: " + input_message +
    ...
    {
      Serial.println("test mode is OFF");
      if(atoi(dhcpEnable)!=0)
      {
        Serial.println("DHCP Enable is ON");
        network.dhcp_enable = true;
      }
      else
      {
        Serial.println("DHCP Mode is OFF");
        network.dhcp_enable = false;
        strcpy(network.ssid,ssid);
        strcpy(network.password,password);
        strcpy(network.ip_wifi,ip);
        strcpy(network.gateway_wifi,gateway);
        strcpy(network.subnet_wifi,subnet);
        strcpy(network.priDNS_wifi,priDNS);
        strcpy(network.secDNS_wifi,secDNS);
        /*
        Serial.print("ssid = ");
        Serial.println(network.ssid);
        Serial.print("Password = ");
